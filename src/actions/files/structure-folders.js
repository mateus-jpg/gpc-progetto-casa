...
export async function getStructureFolderTree(structureId) {
  try {
    ...
    const snapshot = await adminDb.collection('structureFolders')
 ...
 */
export async function getStructureFolderContents({ folderId, structureId = null }) {
  try {
    const { userUid } = await requireUser();
    ...
    if (folderId === null) {
      if (!structureId) throw new Error('structureId is required for root directory');
      await verifyUserPermissions({ userUid, structureId });
    } else {
      const folderDoc = await adminDb.collection('structureFolders').doc(folderId).get();
    ...
    // Subfolders
    const subfoldersSnapshot = await adminDb.collection('structureFolders')
    ...
    // Files
    const filesSnapshot = await adminDb.collection('structureFiles')
      .where('structureId', '==', targetStructureId)
    ...
    if (folderId === null) {
      ...
      while (currentFolder) {
        breadcrumbs.unshift({ id: currentFolder.id, nome: currentFolder.nome, path: currentFolder.path });
        if (currentFolder.parentFolderId) {
          const parentDoc = await adminDb.collection('structureFolders').doc(currentFolder.parentFolderId).get();
          if (parentDoc.exists) {
    ...
    return {
      success: true,
      folder: folderData ? { id: folderId, ...folderData,
 ...
 */
export async function createStructureFolder({ structureId, nome, parentFolderId = null }) {
  try {
    const { userUid, headers } = await requireUser();
    const userEmail = headers.get('x-user-email');
    await verifyUserPermissions({ userUid, structureId });

    if (!nome || nome.trim().length === 0) throw new Error('Folder name is required');
    if (nome.length > 100) throw new Error('Folder name must be 100 characters or less');

    let depth = 0;
    let path = `/${nome.trim()}`;

    if (parentFolderId) {
      const parentDoc = await adminDb.collection('structureFolders').doc(parentFolderId).get();
      if (!parentDoc.exists || parentDoc.data().deleted) throw new Error('Parent folder not found');
      const parentData = parentDoc.data();
      if (parentData.structureId !== structureId) throw new Error('Parent folder belongs to different structure');
      depth = parentData.depth + 1;
      path = `${parentData.path}/${nome.trim()}`;
      if (depth > MAX_FOLDER_DEPTH) throw new Error(`Maximum folder depth of ${MAX_FOLDER_DEPTH} exceeded`);
    }
    ...
    const folderRef = await adminDb.collection('structureFolders').add(folderData);
    ...
    invalidateStructureFolderCaches(structureId, parentFolderId ? [parentFolderId] : []);
 ...
 */
export async function renameStructureFolder({ folderId, newName, structureId }) {
  try {
    const { userUid } = await requireUser();
    if (!newName || newName.trim().length === 0) throw new Error('Folder name is required');
    if (newName.length > 100) throw new Error('Folder name must be 100 characters or less');
    ...
    await verifyUserPermissions({ userUid, structureId: folderData.structureId });
    if (folderData.isDefaultCategory) throw new Error('Cannot rename default category folders');
    ...
    const descendantsSnapshot = await adminDb.collection('structureFolders')
 ...
 */
export async function deleteStructureFolder({ folderId, cascade = false, structureId }) {
  try {
    const { userUid } = await requireUser();
    ...
    await verifyUserPermissions({ userUid, structureId: folderData.structureId });
    if (folderData.isDefaultCategory) throw new Error('Cannot delete default category folders');

    const subfoldersSnap = await adminDb.collection('structureFolders')
      .where('parentFolderId', '==', folderId).where('deleted', '==', false).limit(1).get();
    const filesSnap = await adminDb.collection('structureFiles')
      .where('folderId', '==', folderId).where('deleted', '==', false).limit(1).get();
    ...
    if (cascade && hasContents) {
      const allSubsSnap = await adminDb.collection('structureFolders')
        .where('structureId', '==', folderData.structureId).where('deleted', '==', false).get();
      const descendants = allSubsSnap.docs
        .map(doc => ({ id: doc.id, ...doc.data() }))
        .filter(f => f.path.startsWith(folderData.path + '/'));

      const allFilesSnap = await adminDb.collection('structureFiles')
        .where('structureId', '==', folderData.structureId).where('deleted', '==', false).get();
      const filesToDelete = allFilesSnap.docs
        .map(doc => ({ id: doc.id, ...doc.data() }))
        .filter(f => f.folderId === folderId || descendants.some(d => d.id === f.folderId));
      ...
      for (let i = 0; i < allOps.length; i += 500) {
        ...
        allOps.slice(i, i + 500).forEach(op => {
          batch.update(adminDb.collection(op.type).doc(op.id), {
            deleted: true, deletedAt: new Date(), deletedBy: userUid, updatedAt: new Date(),
          });
        });
        await batch.commit();
    ...
    return { success: true, message: 'Folder deleted successfully', deletedCount };
  } catch (err) {
 ...
 */
export async function moveStructureFolder({ folderId, newParentFolderId, structureId }) {
  try {
    const { userUid } = await requireUser();
    ...
    await verifyUserPermissions({ userUid, structureId: folderData.structureId });
    if (folderData.isDefaultCategory) throw new Error('Cannot move default category folders');

    let newDepth = 0;
    let newPath = `/${folderData.nome}`;

    if (newParentFolderId) {
      const parentDoc = await adminDb.collection('structureFolders').doc(newParentFolderId).get();
      if (!parentDoc.exists || parentDoc.data().deleted) throw new Error('Target parent folder not found');
      const parentData = parentDoc.data();
      if (parentData.structureId !== folderData.structureId) throw new Error('Cannot move folder to different structure');
      if (parentData.path.startsWith(folderData.path)) throw new Error('Cannot move folder into its own descendant');
    ...
    const descendantsSnap = await adminDb.collection('structureFolders')
      .where('structureId', '==', folderData.structureId).where('deleted', '==', false).get();
    ...
    const batch = adminDb.batch();
    const affectedFolderIds = [folderId];
    if (folderData.parentFolderId) affectedFolderIds.push(folderData.parentFolderId);
    if (newParentFolderId) affectedFolderIds.push(newParentFolderId);

    const depthDiff = newDepth - folderData.depth;
    batch.update(adminDb.collection('structureFolders').doc(folderId), {
      parentFolderId: newParentFolderId, path: newPath, depth: newDepth, updatedAt: new Date(),
    });
 ...
 */
export async function moveStructureFileToFolder({ fileId, targetFolderId, structureId }) {
  try {
    const { userUid } = await requireUser();
    ...
    if (targetFolderId) {
      const targetDoc = await adminDb.collection('structureFolders').doc(targetFolderId).get();
      if (!targetDoc.exists || targetDoc.data().deleted) throw new Error('Target folder not found');
    ...
    await logDataUpdate({
      ...
      details: { fileName: fileData.nome, oldFolderId, targetFolderId, targetFolderPath },
    });

    invalidateStructureFolderCaches(fileData.structureId, [oldFolderId, targetFolderId].filter(Boolean));
