...
import {
  addExistingStructureToProject,
  getAvailableStructuresForProject,
...
export function AddExistingStructureToProjectDialog({ projectId }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const result = await addExistingStructureToProject(
        projectId,
        selectedStructureId,
      );
      if (result.success) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Select a structure you administer to add to this project. Only
            structures that are not already assigned to a project are shown.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-2">
            ...
            {fetchingStructures ? (
              ...
              <p className="text-sm text-muted-foreground">
                No available structures to add. You need to be an admin of a
                structure that is not already assigned to a project.
              </p>
            ) : (
              <Select
