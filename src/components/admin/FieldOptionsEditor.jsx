...
export function FieldOptionsEditor({
  ...
  const toggleDefaultOption = useCallback(
    (option, checked) => {
      ...
      if (checked) {
        ...
        newSelected = [
          ...selectedDefaults.filter((o) => o !== option),
          ...customOptions,
        ];
      }
      ...
      onChange(
        newSelected.length === defaultOptions.length &&
  ...
  const addCustomOption = useCallback(() => {
    ...
    if (
      selectedOptions.some((opt) => opt.toLowerCase() === trimmed.toLowerCase())
    ) {
      return;
    }
  ...
  const removeCustomOption = useCallback(
    (option) => {
      ...
      onChange(
        newSelected.length === defaultOptions.length && newCustom.length === 0
          ? null
          : newSelected,
      );
    },
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <Label className="text-sm font-medium">
          Aggiungi Opzione Personalizzata
        </Label>
        <div className="flex gap-2">
          <Input
            value={newOption}
      ...
      <div className="text-xs text-muted-foreground pt-2 border-t">
        {isUsingDefaults ? (
          <span>
            Usando tutte le opzioni predefinite ({defaultOptions.length})
          </span>
        ) : (
          <span>
            {selectedDefaults.length} opzioni predefinite selezionate
            {customOptions.length > 0 &&
              ` + ${customOptions.length} personalizzate`}
          </span>
        )}
      </div>
    </div>
