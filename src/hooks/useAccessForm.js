...
export function useAccessForm(categories = null, initialData = null) {
    ...
    setAccessState((prev) => {
      ...
      return {
        ...
        [typeVal]: {
          ...
          existingFiles: current.existingFiles.filter(
            (f) => f.path !== filePath,
          ),
        },
      };
  ...
  const isAccessTypeValid = useCallback(
    (typeVal) => {
      const s = accessState[typeVal];
      if (!s) return false;
      if (s.subCategories.length > 0) {
        if (s.subCategories.includes("Altro") && !s.altroText.trim())
          return false;
        return true;
      }
      return false;
    ...
    return await Promise.all(
      validTypes.map(async (type) => {
        ...
        if (state.subCategories.length > 0)
          cleanedState.sottoCategorie = state.subCategories;
        ...
        if (state.classification)
          cleanedState.classificazione = state.classification;
        if (state.referralEntity)
          cleanedState.enteRiferimento = state.referralEntity;
        ...
        if (state.files.length > 0) {
          cleanedState.files = await Promise.all(
            state.files.map(async (f) => {
              ...
              return {
                ...
                creationDate:
                  f.creationDate instanceof Date
                    ? f.creationDate.toISOString()
                    : f.creationDate,
                expirationDate:
                  f.expirationDate instanceof Date
                    ? f.expirationDate.toISOString()
                    : f.expirationDate,
                base64,
                type: f.file.type,
                size: f.file.size,
